"""Specification agent — summarises feedback clusters into task descriptions via Ollama."""
...
class SpecifierAgent(Agent):
    """Summarises prioritised feedback clusters into actionable task descriptions."""
    ...
    def run(self, input: AgentInput) -> AgentOutput:
        ...
        if not clusters:
            logger.info("No clusters to specify")
            return AgentOutput(
                data={"tasks": []},
                success=True,
        ...
        if not budget["allowed"]:
            logger.warning("Budget exhausted — skipping specification")
            return AgentOutput(
                data={"tasks": []},
                success=True,
                message="Budget exhausted — no tasks specified",
                tokens_used=0,
            )
        ...
        logger.info("Specified %d task(s) using %d tokens", len(tasks), total_tokens)
        return AgentOutput(
            data={"tasks": tasks},
            success=True,
            message=f"Specified {len(tasks)} task(s)",
